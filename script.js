...
function createSpeciesDetailCard(species) {
    ...
    // Add title header with eBird link
    ...
    if (commonNameText && commonNameText !== 'Unknown') {
        ...
        titleHeader.appendChild(commonNameTitle);
    }

    // Add eBird link in header
    if (species.speciesCode) {
        const ebirdLink = document.createElement('a');
        ebirdLink.className = 'ebird-link';
        ebirdLink.href = `https://ebird.org/species/${species.speciesCode}`;
        ebirdLink.target = '_blank';
        ebirdLink.rel = 'noopener noreferrer';
        ebirdLink.innerHTML = 'eBird <i class="fas fa-arrow-right"></i>';
        titleHeader.appendChild(ebirdLink);
    }
